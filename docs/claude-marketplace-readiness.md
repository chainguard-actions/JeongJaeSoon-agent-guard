...
| Network and telemetry | Undisclosed non-MCP outbound calls or default-on telemetry fail review. Downloads must be visible to the reviewer. | **Ready with declared flags.** Normal hooks are local and there is no telemetry. Checksum/install calls go only to GitHub Releases; the experimental HTTP PII adapter is off by default and fully disclosed. It has no service-specific compatibility guarantee. Expected scan flags: `may_make_external_network_calls=true`, `may_download_additional_software=true`. |
