...
tree plus the one file a write tool just targeted. Default hook processing is
local: Agent Guard has no telemetry, developer
...
The updater validates the downloaded payload before extraction and preserves the
public bin directory used to invoke it, which reduces the failure surface but
does not make the whole installation transaction atomic.

Standalone v3.1.1 shipped `update` with a symlink failure that leaves `version`
and `doctor` reporting `Too many levels of symbolic links`; v3.2.0 fixes it. Do
not run the broken version's `update` to repair itself — reinstall with
`bootstrap.sh` from a current release instead. Recovery steps for an install
already in that state are in
...
Agent Guard is a deterministic, thin guardrail — not a DLP system, EDR, or vault. It scans tracked diffs, staged changes, untracked files, and the file a write tool just targeted with gitleaks, and blocks a fixed list of sensitive paths and shell idioms. It deliberately does **not** inspect arbitrary file contents that a command reads, and it has these blind spots by design:

- **Gitignored files are only scanned when a write tool just targeted them.** The working-tree backstop uses `git ls-files --others --exclude-standard` and `git diff`, both of which skip `.gitignore`d paths. `PostToolUse` closes that for the path a `Write`, `Edit`, `MultiEdit`, or `NotebookEdit` call names — that one file is read and scanned in full, so a secret written into `secrets/` or `*.local` is caught. Everything else about an ignored file stays invisible: content that arrived through `Bash`, an MCP tool, or an `apply_patch` envelope (none of which name a path Agent Guard can key on), and any secret already sitting in an ignored file nobody wrote to this session. Agent Guard never sweeps ignored files on its own — reporting the operator's own local credential files on every write is how a scanner gets turned off. Use `agent-guard scan-path <dir>` for an on-demand sweep, and keep real secrets out of the repo entirely. Tracked files held out of the index with `git update-index --skip-worktree` or `--assume-unchanged` are invisible to `git diff HEAD` in the same way, and get the same `PostToolUse` coverage and the same limits.
- **Outside the git work tree, only the file a write tool just targeted is covered.** The working-tree scan no-ops outside a git repository and is scoped to the current repo, so nothing there gets the diff/untracked backstop. The `PostToolUse` path scan still applies: a `Write`/`Edit`/`MultiEdit`/`NotebookEdit` target is scanned whether it sits outside every repository or inside a *different* one, and a symlink is resolved first so it cannot launder its target. Only regular files are read; a symlink to a device or fifo is skipped. A target over 10 MiB, or one that cannot be read, is an infrastructure failure and follows `AGENT_GUARD_INFRA_FAILURE_MODE` rather than reporting a clean scan. Files written outside the repo by `Bash` or an MCP tool still get no backstop — use `agent-guard scan-path <dir>` to scan an arbitrary tree on demand.
...
Wiring the hooks yourself, without installing the plugin? `examples/` holds a
working manifest per host — [`examples/claude/settings.project.json`](examples/claude/settings.project.json),
[`examples/codex/hooks.json`](examples/codex/hooks.json), and a CI workflow in
[`examples/github/secret-guard.yml`](examples/github/secret-guard.yml). The two
hook manifests are generated by `scripts/render-hook-manifests.sh` from the same
tables as the shipped plugin manifests, so their event coverage and timeouts
cannot drift from the plugin; edit the renderer, not the files.
...
| Standalone update reports a symlink loop | v3.1.1 only, fixed in v3.2.0. Reinstall with a current `bootstrap.sh` rather than repairing in place; recovery steps are in [#194](https://github.com/JeongJaeSoon/agent-guard/issues/194). Do not delete the old installation blindly. |
